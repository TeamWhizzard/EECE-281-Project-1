...
void automaticDrivingMode() {
  ...
  /*if (millis() > (lcdTime + 200)) {
    //delay(500);
    createAutoBluetoothMessage(0, (distanceCm * 0.33)); // converts distance to percentage
    lcdTime = millis();
  }*/
...
void turnLeftAuto() {
  ...
  //if (millis() > (lcdTime + 200)) {
    //createAutoBluetoothMessage(0, 0);
    //lcdTime = millis();
  //}
...
void demoLCD(int dir) { // 1 forward, 0 left
  if ((printTimer + 500) < millis()) { // delay print for readability
    ...
    if (dir == 1) {
      ...
      lcd.print("*****FORWARD****");
      lcd.setCursor(0,1);
      lcd.print("    " + String(distanceCm * 0.33) + "%"); // converts distance to percentage
    } else {
      lcd.setCursor(0,0);
      lcd.print("//////LEFT//////");
      lcd.setCursor(0,1);
